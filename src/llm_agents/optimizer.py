...
# ==============================================================================
# 0. Global & Visual Configuration (Nature/Science Ultimate Edition - Large Fonts & No Title)
# ==============================================================================

RDLogger.DisableLog('rdApp.*')
warnings.filterwarnings('ignore')

# --- [Modified] Font settings fully consistent with Scout ---
...
# --- [Modified] Font sizes significantly increased (Sync with Scout) ---
mpl.rcParams['axes.labelsize'] = 48    # Axis labels (extra large)
mpl.rcParams['xtick.labelsize'] = 40   # Tick labels
mpl.rcParams['ytick.labelsize'] = 40
mpl.rcParams['legend.fontsize'] = 34   # Legend font size
mpl.rcParams['font.size'] = 34         # Global default font size

# --- [Modified] Line thickness increased (Sync with Scout) ---
mpl.rcParams['axes.linewidth'] = 4.0      # Outer borders thicker
...
mpl.rcParams['grid.linewidth'] = 2.0      # Grid lines slightly thicker (required for 3D plots)
...
class OptimizerAgent:
    def __init__(self):
        ...
        self.img_dir = os.path.join(get_run_dir(), "images") # Store figures uniformly in images
    ...
    def _get_raw_fp_vector(self, smiles):
        """[Performance Optimization] Get the raw fingerprint vector of a single molecule (moles=1.0) for pre-computed linear scaling"""
        vec = np.zeros(2048, dtype=float)
        if not smiles: return vec
        mol = Chem.MolFromSmiles(smiles)
        if not mol: return vec
        fp_gen = AllChem.GetHashedMorganFingerprint(mol, radius=2, nBits=2048)
        for bit, count in fp_gen.GetNonzeroElements().items():
            if bit < 2048: vec[bit] = count
        return vec
    ...
    def _flatten_input_vector(self, batch_tuple):
        """Flatten batch tuple into a single-level dictionary for CSV saving"""
    ...
    # ==========================================================================
    # Plotting Logic (Modified for High-Impact Journal Style - Big Fonts, No Title)
    # ==========================================================================
    def _batch_plot_all_surfaces(self, df_plot, task_type, best_meta):
        ...
        if len(valid_cols) < 2:
            print("⚠️ Insufficient valid varying parameters (less than 2), skipping plotting.")
            return
        
        combinations = list(itertools.combinations(valid_cols, 2))
        print(f"   -> Filtered active dimensions based on recipe type: {valid_cols}")
    ...
    def _plot_single_surface(self, df, x_col, y_col, param_map, task_type, best_meta):
        """
        [Modified] Single plot rendering: Scout style - Fix axis text truncation issues
        1. Increase figsize
        ...
        try:
            ...
            # [Modification 1] Canvas size increased from (12, 11) to (16, 14) to leave room for large fonts
            ...
            ax.set_box_aspect((1, 1, 0.7)) # Flatten slightly
            ...
            # --- [Key Modification 3] Axis label setup ---
            # labelpad increased from 30 to 60 (or larger) to force labels away from axes
            ...
            # --- [Key Modification 4] Ticks setup ---
            # pad=15 keeps tick values farther from axis line to avoid colliding with axis label
            ...
            # --- [Key Modification 5] Manual margins before saving ---
            # Even with bbox_inches='tight', standard 3D plots might not calculate boundary for large fonts correctly
            # So we can make the layout slightly looser
            plt.subplots_adjust(left=0.1, right=0.9, bottom=0.1, top=0.9)

            save_path = os.path.join(self.img_dir, save_name)
            
            # bbox_inches='tight' coupled with pad_inches=0.5 ensures extra space is reserved during clipping
            plt.savefig(save_path, bbox_inches='tight', pad_inches=0.5, transparent=True)
            plt.close()
            print(f"✅ [Vis] 3D plot saved (Nature Style - Fixed Layout): {save_path}")
        except Exception as e:
    ...
    def optimize(self, summary_report, candidates):
        ...
        # 2. Search Loop
        N_ITER = 300 
        RATIOS = [1.0, 3.0, 5.0, 10.0, 15.0, 20.0]
        BIT_NAMES = [f"Bit_{i}" for i in range(2048)]  # [Optimization] Pre-generate key list
        DESC_TEST_KEYS = ['C','H','O','N','Other_atoms','TPSA','Amino','Amide Group','-COOH','-OH','C=O','-C≡N','-SO₃H','Log P','Melting Point','Boiling Point']
        
        # Containers to hold full input features
        ...
        # [Optimization] Pre-calculate zero descriptors (previously recalculated every iteration -> now only once)
        desc_zeros = self._calculate_mol_descriptors(None, 0)

        # [Optimization] Pre-calculate fingerprints and descriptors cache for each matrix
        mat_fp_cache = {}
        mat_desc_cache = {}  # key: (mat_smiles, ratio) -> desc_dict
        for mat in COMMON_MATRICES:
            ...
            for r in RATIOS:
                ...
                for k, v in desc_unit.items():
                    if k == 'Molecular Weight': d[k] = v
                    elif k == 'Molar Amount': d[k] = r
                    else: d[k] = v * r
        ...
        for mol_data in candidates:
            ...
            fp_raw_candidate = self._get_raw_fp_vector(c_smiles)  # [Optimization] Pre-calculated once
            
            for mat in COMMON_MATRICES:
                mat_smiles = mat['SMILES']; mat_name = mat['Name']
                fp_raw_matrix = mat_fp_cache[mat_smiles]  # [Optimization] Retrieved from cache
                batch_rows = []; batch_meta = [] 

                # [Optimization] Vectorized random parameter generation (300x random.choice -> 1x numpy)
                v_ratios = np.random.choice(RATIOS, size=N_ITER)
                v_methods = np.random.choice([1, 2], size=N_ITER)
                v_s1_temps = np.round(np.random.uniform(100, 850, N_ITER), 1)
                v_s1_times = np.round(np.random.uniform(1, 800, N_ITER), 1)
                v_s1_rxns = np.random.choice([1, 2, 3, 4, 5], size=N_ITER)
                v_dosages = np.round(np.random.uniform(0, 10, N_ITER), 2)
                v_s2_rxns = np.random.choice([1, 2, 5], size=N_ITER)
                v_s2_temps = np.round(np.random.uniform(100, 850, N_ITER), 1)
                v_s2_times = np.round(np.random.uniform(1, 800, N_ITER), 1)

                for iter_idx in range(N_ITER):
                    ratio = float(v_ratios[iter_idx])
                    method = int(v_methods[iter_idx])
                    s1_temp = float(v_s1_temps[iter_idx]); s1_time = float(v_s1_times[iter_idx]); s1_rxn = int(v_s1_rxns[iter_idx])
                    
                    if method == 1: dosage = 0.0; s2_rxn = 0; s2_temp = 0.0; s2_time = 0.0
                    else: dosage = float(v_dosages[iter_idx]); s2_rxn = int(v_s2_rxns[iter_idx]); s2_temp = float(v_s2_temps[iter_idx]); s2_time = float(v_s2_times[iter_idx])

                    # [Optimization] Retrieve descriptors from cache (eliminates 300x RDKit calls)
                    desc_pre2 = mat_desc_cache[(mat_smiles, ratio)]
                    # [Optimization] Linearly scale fingerprints (eliminates 300x fingerprint calculations)
                    fp_vec = fp_raw_candidate + ratio * fp_raw_matrix
                    desc_test = {k: desc_pre1_base.get(k, 0) + desc_pre2.get(k, 0) for k in DESC_TEST_KEYS}

                    row = dict(zip(BIT_NAMES, fp_vec))  # [Optimization] dict(zip) is faster than comprehension
                    ...
                    # Collect input features (Flatten) for CSV saving
                ...
                for idx in range(len(batch_rows)):
                    ...
                    # Collect prediction results for CSV saving
        ...
        # ======================================================================
        # 💾 Core Feature: Save 2199-dimensional model inputs + predictions CSV
        # ======================================================================
        print("💾 Constructing full model input dataset (containing 2199 features)...")
        if all_flat_inputs_list:
            # 1. Get standard feature names (retrieved from Loaded Context, ensuring 2199 dimensions)
            # Prioritize emission feature names; if absent, use lifetime
            std_features = []
            ...
            if std_features:
                try:
                    ...
                    # 3. Force alignment: reorder according to standard feature names, fill missing with 0 (critical step)
                    df_aligned = df_features.reindex(columns=std_features, fill_value=0.0)
                    
                    # 4. Build prediction results DataFrame
                    ...
                    print(f"   -> Shape: {df_final.shape} (rows x [metadata + predictions + 2199 features])")
                except Exception as e:
                    print(f"⚠️ Saving model input CSV failed: {e}")
            else:
                print("⚠️ Unable to obtain standard feature names list, skipping save.")

        # 4. Save general history
        if all_meta_list:
            pd.DataFrame(all_meta_list).to_csv(os.path.join(self.log_dir, "Total_Search_History.csv"), index=False)

        # 5. Return and Plotting
        print("\n" + "="*50)
