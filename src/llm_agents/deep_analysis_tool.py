# src/llm_agents/deep_analysis_tool.py
...
def preprocess_data_for_model(df):
    """Replicate the preprocessing logic from model training"""
...
# ==============================================================================
# 3. Core Analysis Logic Functions
# ==============================================================================
...
def run_pdp_analysis(model_path, data_path, features, output_dir, task_type):
    ...
    if not valid_features:
        return pdp_failures + ["No valid features to plot"]
    ...
    print(f"    Plotting {len(valid_features)} PDP charts...")
    ...
    for feat in valid_features:
        try:
            ...
            pdp_failures.append(f"Plotting failed ({feat}): {str(e)}")
            plt.close(fig)

    print(f"✅ PDP analysis completed.")
    return pdp_failures

def decode_bit_structures(bit_list, smiles_file, target_col_idx_1based, output_dir, task_type):
    print(f"\n>>> [2/2] Starting Bit Substructure Decoding (Algorithm: Hashed Morgan Count)...")
    
    if not os.path.exists(smiles_file): 
        print(f"❌ SMILES file does not exist: {smiles_file}")
        return []
    ...
    try:
        ...
        print(f"❌ Failed to read SMILES data: {e}")
        return []
    ...
    for bit_name, bit_id in bit_list:
        ...
        try:
            ...
            # --- [Fix] Draw directly to PNG using RDKit, avoiding reliance on external browser conversion ---
            try:
                d_cairo = rdMolDraw2D.MolDraw2DCairo(500, 500)
                opts_cairo = d_cairo.drawOptions()
                opts_cairo.explicitMethyl = True   
                opts_cairo.legendFontSize = 24
                opts_cairo.bondLineWidth = 3.0
                opts_cairo.padding = 0.1
                d_cairo.DrawMolecule(submol, legend=legend_text)
                d_cairo.FinishDrawing()
                with open(os.path.join(bit_dir, f"{bit_name}_substructure.png"), 'wb') as f:
                    f.write(d_cairo.GetDrawingText())
            except Exception as e:
                print(f"⚠️ MolDraw2DCairo failed, attempting MolToImage: {e}")
                try:
                    from rdkit.Chem import Draw
                    img = Draw.MolToImage(submol, size=(500, 500), legend=legend_text)
                    img.save(os.path.join(bit_dir, f"{bit_name}_substructure.png"))
                except Exception as e2:
                    print(f"⚠️ MolToImage also failed: {e2}")
            ...
            err_msg = f"Plotting error ({bit_name}): {str(e)}"
            bit_errors.append(err_msg)

    print(f"✅ Bit substructure decoding completed for {task_type}.")
    return bit_errors
...
class DeepAnalysisRunner:
    ...
    def __init__(self):
        ...
        # Using a sub-directory "Deep_Analysis" within the unified run directory
        ...
        print(f"📂 Analysis results will be saved to: {self.output_base_dir}")
