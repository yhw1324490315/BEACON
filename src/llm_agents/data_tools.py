...
# =====================================================================
# COLUMN MAPPING (Unicode escapes preserve raw compatibility with Chinese headers)
# =====================================================================
COLUMN_MAPPING = {
    "EMISSION_PEAK": '\u53d1\u5c04\u6ce2\u957f (nm)',       # Unicode escape for 'Emission Wavelength (nm)'
    "LIFETIME_MS":   '\u4f59\u6656\u5bff\u547d\uff08ms\uff09',      # Unicode escape for 'Lifetime (ms)'
    "FEATURE_COL":   'Feature',       
...
class DataToolkit:
    ...
    def _load_data(self):
        ...
        def read_file(key_name):
            ...
            if not path or not os.path.exists(path): 
                if path: print(f"⚠️ File not found: {path}")
                return None
        ...
        print(">>> [DataToolkit] Loading completed.")

    def get_experiment_data_with_sampling(self, target_type: str, min_val: float = None, max_val: float = None):
        """Retrieve experimental records using a layered sampling algorithm"""
        df = self.data_store.get('exp')
        if df is None: return {"error": "No experimental records found."}
        ...
        if col_name not in df.columns: return {"error": f"Column '{col_name}' does not exist"}

        df_clean = df.dropna(subset=[col_name]).sort_values(by=col_name)
        if df_clean.empty: return {"warning": "No valid values in this column"}

        final_rows = []
        
        # Emission Peak target: global 30nm windows + local focus ±50nm window
        ...
        # Lifetime target: optimized sampling strategy (Top 20 Best + Gradient Sampling)
            ...
            if target_df.empty:
                return {"warning": "No records found matching criteria after filtering."}

            # Get Top 20 best performing records
            top_20 = target_df.head(20)
            final_rows.append(top_20)
            
            # Gradient sampling for the remaining records
        ...
        if not final_rows: return {"warning": "No data matching sampling constraints found."}
    ...
    def query_feature_importance(self, target: str, top_n: int = 8):
        """Query global SHAP feature importance metrics"""
